...
sealed class Screen(val route: String) {
    ...
    object CreateStory   : Screen("write/new_story")
    object CreateEpisode : Screen("write/episode/{storyId}/{chapterNum}") {
    ...
    // Stub screens — visible but non-functional in MVP
...
fun KathakarNavGraph(navController: NavHostController) {
    ...
    NavHost(
        ...
        composable(Screen.StoryDetail.route,
            listOf(navArgument("storyId") { type = NavType.StringType })
        ) { back ->
        ...
        composable(Screen.EpisodeReader.route,
            listOf(navArgument("episodeId") { type = NavType.StringType })
        ) { back ->
        ...
        composable(Screen.Write.route) {
            ...
            WriteScreen(
                ...
                onCreateEpisode = { storyId, ch -> navController.navigate(Screen.CreateEpisode.go(storyId, ch)) },
        ...
        composable(Screen.CreateEpisode.route, listOf(
            ...
            val storyId = back.arguments?.getString("storyId") ?: return@composable
            val ch      = back.arguments?.getInt("chapterNum") ?: 1
            val user    = auth.user ?: return@composable
            CreateEpisodeScreen(
                storyId       = storyId,
                chapterNumber = ch,
        ...
        composable(Screen.BuyCoins.route) {
            ComingSoonScreen("Buy Coins",
                "Payments are not enabled in this preview build.\nYou received ${com.kathakar.app.domain.model.MvpConfig.FREE_COINS_ON_SIGNUP} free coins on sign-up to test the platform!",
                onBack = { navController.popBackStack() })
        }
        composable(Screen.Subscribe.route) {
            ComingSoonScreen("Subscriptions",
                "Subscription plans (₹99/month all-access, ₹25/month per-author) will be enabled in the next release.",
                onBack = { navController.popBackStack() })
        }
        composable(Screen.AiWrite.route) {
            ComingSoonScreen("AI Writing Assistant",
                "AI-powered story generation and refinement is coming soon.\nWrite your stories manually for now.",
                onBack = { navController.popBackStack() })
        }
    }
}
