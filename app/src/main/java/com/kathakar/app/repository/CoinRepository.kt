        ...
        return try {
            ...
            db.runTransaction { t ->
                ...
                t.update(authorRef, "coinBalance", FieldValue.increment(credit.toLong()),
                    "totalCoinsEarned", FieldValue.increment(credit.toLong()))
                t.set(unlockRef, UnlockedEpisode(userId, episode.episodeId, episode.storyId, cost, Timestamp.now()))
                t.set(txnRef, CoinTransaction(txnRef.id, userId, CoinTxnType.EPISODE_UNLOCK, -cost, "Unlocked: ${episode.title}", episode.episodeId, Timestamp.now()))
                t.set(aTxnRef, CoinTransaction(aTxnRef.id, authorId, CoinTxnType.AUTHOR_EARNING, credit, "Earned: ${episode.title}", episode.episodeId, Timestamp.now()))
            }.await()
